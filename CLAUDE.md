...
- **Quotation Audit** — scans every library `.md` (excludes `series/Narrative Journey Series/Foundations/Test Book/`), compares each verbatim-intent scripture quotation to the current stored BSB, and streams collapsible per-book cards (cover art + counts). Each finding shows the session H1 (linked), the surrounding paragraph with the quote highlighted + differing words bold, the full BSB verse, and a **"Fix quote"** button whose replacement text is an **editable `<textarea>`** pre-filled with the computed wording (adjust before applying; blockquote quotes are re-wrapped with `> `).
...
- `src/server/bible-quote-audit.js` — `runStreamingQuoteAudit`, `scanText` (findings), `correctQuote`/`computeFix` (BSB wording for the quoted span; falls back to the full verse when there's no clean span).
