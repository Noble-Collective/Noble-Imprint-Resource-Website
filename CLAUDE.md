...
**Unified reader identity (`AUTH_UNIFIED`):** admin/editor login historically used project `noble-imprint-website`; the reader user-data layer uses `noble-imprint-463519` (the shared convergence project). With `AUTH_UNIFIED=1` (on in prod) both collapse to one 463519 identity: `auth.js` uses a named `auth463519` admin app (keyless `serviceAccountId` impersonation) for `verifyIdToken`/`createSessionCookie`/`verifySessionCookie`, while the default app keeps `noble-imprint-website` Firestore/data. All authz keys on `req.user.email` (roles, super-admin, attribution) — NOT Firebase uid — so the identity-project switch is low-risk. Flag off = the old two-login behavior; `/api/auth/test-login` (`__dev_auth`) is project-independent so the Playwright suite is unaffected either way.
...
- **E2E:** gitignored `test-harness/` (Playwright + Firebase emulators; `?ncEmu=1` + `window.__ncTestSignIn` localhost-only seam) exercises the whole reader incl. 2-tab live-propagation. Unit tests: `npm run test:unit`.
