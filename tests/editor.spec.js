...
async function clearAllComments() {
  ...
  if (!admin.apps.length) admin.initializeApp();
  const db = admin.firestore();
  const snap = await db.collection('comments').where('filePath', '==', TEST_FILE).get();
...
async function cleanupBotData() {
  ...
  for (const col of ['suggestions', 'comments', 'replies']) {
    const snap = await db.collection(col).where('authorEmail', '==', botEmail).where('filePath', '==', TEST_FILE_PATH).get();
    if (!snap.empty) {
      const batch = db.batch();
      snap.docs.forEach(d => batch.delete(d.ref));
      await batch.commit();
