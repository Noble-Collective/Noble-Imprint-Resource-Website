...
/* Inline marker before the first word (shifts text, always on-screen). It's inserted after the async
   annotation load, so to avoid a visible reflow ("the text jumps") the bookmarked line is pre-marked
   from a localStorage cache during boot — see initAnnotations. */
.nc-bm-marker{color:var(--nc-accent);margin-right:.2em;cursor:pointer;user-select:none;vertical-align:middle}
.nc-bm-marker svg{width:15px;height:15px}
