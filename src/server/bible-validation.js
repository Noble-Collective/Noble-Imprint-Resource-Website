...
// Full book name → 3-letter USFM code (for resolving a "Book Ch:V" ref to its USFM file).
...
// "Jude 1:11" → "JUD" (the USFM code); returns null if the book name isn't recognized.
function refToBookCode(ref) {
  const m = String(ref).match(/^(.+?)\s+\d+:\d+/);
  return m ? (BOOK_NAME_TO_CODE[m[1]] || null) : null;
}
...
// Verse text the audiobook renders from the USFM: \d superscription folded into verse 1,
// section headings / speaker labels / acrostic labels skipped (a verse ends only at the next
// \v or \c — in poetry a heading can sit mid-verse), footnotes + inline markers stripped.
// Returns { "ch:vs": text }.
function extractUsfmVerses(usfm) {
  const out = {};
  const lines = String(usfm).split(/\r?\n/);
  let ch = 0, curKey = null, buf = [], pendingD = '';
  const flush = () => { if (curKey) out[curKey] = buf.join(' ').replace(/\\f\s[\s\S]*?\\f\*/g, ' ').replace(/\\[a-z]+\d*\*?/g, ' ').replace(/\s+/g, ' ').trim(); };
  for (const l of lines) {
    const cm = l.match(/^\s*\\c\s+(\d+)/); if (cm) { flush(); curKey = null; buf = []; pendingD = ''; ch = parseInt(cm[1], 10); continue; }
    const dm = l.match(/^\s*\\d\s+(.+)$/); if (dm) { pendingD = dm[1]; continue; }
    const vm = l.match(/^\s*\\v\s+(\d+)\s?([\s\S]*)$/);
    if (vm) { flush(); curKey = ch + ':' + vm[1]; buf = [(pendingD ? pendingD + ' ' : '') + vm[2]]; pendingD = ''; continue; }
    if (!curKey) continue;
    if (/^\s*\\(s\d?|r|ms\d?|mr|qa|sp|sr|cl|cp)\b/.test(l)) continue;            // heading/label — not verse text, verse stays open
    if (/^\s*\\(q|p|b|li|m|nb|pi|pc|qr|qc)/.test(l)) buf.push(l.replace(/^\s*\\[a-z]+\d*\s?/, ''));
    else if (!/^\s*\\/.test(l)) buf.push(l);                                     // plain continuation line
    // any other marker: skip but keep the verse open
  }
  flush();
  return out;
}

// Comparison normalization for the two verse stores — folds the cosmetic differences between
// references.json and the USFM (space around dashes, space before closing quotes/punctuation)
// so only a real word difference registers.
function storeCmp(t) {
  return normalizeVerse(String(t).replace(/\s*([—–])\s*/g, '$1')).replace(/\s+(["',.;:!?)\]])/g, '$1').replace(/\s+/g, ' ').trim();
}

// Cross-check our two verse stores: references.json (what the reader serves) vs the USFM \v
// text (what the audiobook renders). Catches a verse edited in one store but not the other.
// oursVerses is a Map "Book Ch:V"→text; usfmFiles is a Map filename→content.
function crossCheckStores(oursVerses, usfmFiles) {
  const byCode = {};
  for (const [name, content] of usfmFiles) byCode[bookCode(name)] = extractUsfmVerses(content);
  let checked = 0, matched = 0;
  const diverged = [];
  for (const [ref, refText] of oursVerses) {
    const code = refToBookCode(ref); if (!code || !byCode[code]) continue;
    const m = ref.match(/(\d+):(\d+)\s*$/); if (!m) continue;
    const utext = byCode[code][m[1] + ':' + m[2]]; if (utext == null) continue;
    checked++;
    if (storeCmp(utext) === storeCmp(refText)) matched++;
    else if (diverged.length < 100) diverged.push({ ref, references: refText, usfm: utext });
  }
  return { checked, matched, diverged: diverged.length, samples: diverged.slice(0, 25) };
}
